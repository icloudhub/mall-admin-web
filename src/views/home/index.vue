...
import { selectCount } from "@/api/home";
...
export default {
  ...
  methods: {
    ...
    // get
    formatStatus(value){
      //0->待付款；1->待发货；2->已发货；3->待收货（待自提） 4->已完成；5->已关闭；6->无效订单
        if (value === 0) {
          return '待付款';
        } else if (value === 1) {
          return '待发货';
    ...
    //请求
    getorderCount() {
      this.orderCountParam.startDate = formatDate(
        this.orderCountDate[0],
        "yyyy-MM-dd"
      );
      ...
      alert(JSON.stringify(this.orderCountParam));
      selectCount(this.orderCountParam).then(response => {
        let list = response.data;
        this.orderCountresult = list;
      });
    }
  }
};
